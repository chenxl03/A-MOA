...
for i=1:253
    for j=1:253
        ...
        for k=1:7
            error_ra(i,j,k)=error_rt8apx2a(d8r_bin(k,:),d8r_bin(k+1,:));
        end
    end
end

% bb er
sum(sum(sum(error_ra>0)))/(253*253*6)
% bb ED>1
% stall
sum(sum(sum(error_ra>1,3)>0))/(253*253)
...
for i=1:253
    for j=1:253
        ...
        for k=1:7
            error_ga(i,j,k)=error_rt8apx2a(d8g_bin(k,:),d8g_bin(k+1,:));
        end
        ...
        for k=1:7
            error_aa(i,j,k)=error_rt8apx2a(d8a_bin(k,:),d8a_bin(k+1,:));
        end
